...
- **已发版**：release **id=128** / 1.20.29 / versionCode **286** / status=published；
  下载地址 `https://box.hpa888.top/updates/box/android/release/box-1.20.29-286.apk`；
  27,099,696 字节，sha256 `b314a8d53e624c5524d404816125bfdada7b3cc4250ce12cb141524cd41b31e9`，
  清单 HMAC `5499601f…`；285（id=127）已 archived。
- 流程：前台 scp 等完 + 两端 sha256 核对 → dry-run → 真发布（单次成功，未出现 285 那次"半截文件"的问题）。
- **独立五路复核全部通过**：① 公网包 27,099,696 字节与本地逐字节一致；② 按 `version.json` 的
  19 个签名字段自己重算 HMAC 与服务端一致；③ 仓库验签脚本 `线上清单: 1.20.29 (286) passed=true`；
  ④ 客户端 285 → `hasNewVersion=true`、286 → `false`；⑤ 服务端 id=128 published 且字节/sha 对得上、
  id=127 已归档。另在**线上包**上复算中文标记：7 个新标记 0→1 全中、6 个控制项计数不变。
