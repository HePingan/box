...
## 12. D9 / D10 执行记录（284 后续拍板「d8-d10」）

| 项 | 提交 | 实施要点与偏差 |
|---|---|---|
| D9 上传整个文件夹 | `c35a5b4` | 入口放顶栏「更多」（顶栏图标已不少）；选目录用 `FilePicker.getDirectoryPath()`，并加 `debugSetRemoteStorageRuntime(pickDirectory:)` 测试接缝；递归扫描跳过 `.` 开头的隐藏条目（`.nomedia`/`.trashed-*`/`.thumbnails`）并计数、单目录读不出来跳过并计数、文件 500 / 目录 200 上限并明示；确认框摆出文件数/体积/要建的目录数/冲突数；**先按层建目录再传**（父先子后、已存在跳过、405/409 当成功）；每个文件落到自己的子目录，保持结构 |
| D10 跨目录搜索 | `4da6806` | 「搜子目录」入口给在**本目录筛空时的空状态**里（也要在非空列表的筛选横幅上）；客户端逐目录遍历，目录 120 / 结果 200 上限、可取消；进度弹窗用 `ValueNotifier` + `ValueListenableBuilder` 推进（弹窗是另一条 route，页面的 setState 推不动它）；结果面板写明"扫描 N 个目录、命中 M 个"、上限/取消/零结果各有说明；点一条结果跳到它所在的目录 |

**D9/D10 期间发现的真问题**：

1. **自引用/环状目录列表会让遍历绕圈**（服务端把集合自身列出来、或两个目录互指）：
   - 搜索侧表现：`扫了 101 个目录、200 条命中全是重复项`（靠目录上限才停下来）；
   - 递归下载侧表现：同一个文件被下两遍（第二遍还会另存成 `xx (1).jpg`）。
   修法：两个遍历都加 `visited` 目录去重，结果再按 `entry.path` 去重，并各配一条
   "绕圈"用例（断言 `dirsScanned` 不爆炸、结果不重复）。
2. **不定进度圈会让 `pumpAndSettle` 必然超时**（D9 踩到：`_uploading` 期间顶栏是
   `CircularProgressIndicator`）→ widget 测试改用有界 `pump` 循环。
3. **弹窗里的进度更新不能用页面的 `setState`**：弹窗是另一条 route，得让弹窗自己监听
   （`ValueListenableBuilder`）。
4. `exists()` 走的是 **HEAD**（405/501 才退回 PROPFIND）——假服务器只答 PROPFIND 时
