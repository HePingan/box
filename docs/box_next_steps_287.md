...
- 本地构建：**27,181,808 字节**，sha256 `b4609d3a547e26f9212e474eab1ded492672e2c64508155b9b0465e671537d40`，
  符号表 `/root/.box-symbols/287`，验签密钥指纹 `fc9d22015158`。
- 服务端：**release id=129 / v1.20.30 / versionCode 287 / published**，
  `https://box.hpa888.top/updates/box/android/release/box-1.20.30-287.apk`；上一版 id=128(286) 转 archived。
- 上传规矩照旧：**前台 scp 等完 + 两端 sha256 相等**才跑 dry-run，dry-run 与本地包一致（27,181,808 / `b4609d3a…`）
  才真发布；服务端临时包发布后即刻删除。
- 五路独立复核（`/root/.hermes/cache/scratch/verify_publish287.py`，不信发布脚本自述）全部通过：
  ① 公网下载 27,181,808 字节与本机构建**逐字节一致**；
  ② 按 `version.json` 的 **19 个**签名字段自算 HMAC `9a180649a0a9dbc5…` 与服务端一致；
  ③ 仓库客户端验签 `线上清单: 1.20.30 (287) passed=true`；
  ④ 客户端两端 285/286 → `hasNewVersion=true`、287 → `false`；
  ⑤ 服务端 id=129 published（字节/sha 对得上）、id=128 archived。
  线上包 AOT 复算：**9 个本轮新标记 0→1**（全部类型 / 小于 1 MB / 大于 100 MB / 按类型筛选 /
  按修改时间筛选 / 快照已经很久没更新了 / 清除筛选 / 等待 Wi-Fi / 开始不通）、`同时传 ` 0→1、
  5 个控制项计数不变。
- **复核脚本第一版有一条断言写错**：把 `最近 7 天` 当"应增加"的标记，实测 2→2 而报红。
  原因：该字面量（以及 `今天`）在 286 里就已存在（admin_widgets 的日期筛选），**同一份 AOT 快照里
  重复出现的同一个字符串不会让计数再涨**，所以它根本证不了新代码。改用只在本轮出现的
  `最近 30 天` / `按修改时间筛选` 做证据（都 0→1），并在脚本里把这两个字面量标注为"不作证据"。
