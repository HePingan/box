...
class QuizAnswerAligner {
  ...
  /// 驾考高频同义簇（可继续扩充）
    ...
    static QuizAnswerAlignment align({
      ...
      if (letter != null) {
        ...
        if ((bankOptImage || bankSetImage) && surfaceTextLike) {
          // 图选题答案不能贴到文字卷面
        } else if (index >= 0 && index < options.length) {
          final opt = options[index].trim();
          // 仅在无卷面试捕、或卷面同为图选题、或正文已一致时，才允许按字母映射。
          final canLetterMap = (!hasProbeSurface && pureLetter) ||
              bankOpt.isEmpty ||
              surfaceImageLike ||
      ...
      var bankResolved =
          _stripAnswerPrefix(_resolveAgainstBankOptions(raw, bankOptions));
    ...
    static List<int>? _extractMultiIndices(String raw, int optionCount) {
      ...
      // A+B / A、B / AB / A B
      final letters = RegExp(r'[A-Ha-hＡ-Ｈ]').allMatches(t).map((m) {
  ...
  static String _resolveAgainstBankOptions(String raw, List<String> bankOptions) {
    final letter = _extractLetter(raw);
    if (letter != null) {
      final index = letter.codeUnitAt(0) - 'A'.codeUnitAt(0);
  ...
  static Set<String> _clusterKeys(String normText) {
    ...
    for (var i = 0; i < _synonymClusters.length; i++) {
      ...
      for (final term in cluster) {
        ...
        if (normText == tn ||
            normText.contains(tn) ||
            tn.contains(normText)) {
          keys.add('c$i');
          break;
        }
      }
